...
def load_branch_list():
    """
    โหลดรายชื่อสาขาจากชีท "รายชื่อสาขา" ผ่านลิงก์ CSV สาธารณะ (ไม่ต้องใช้ Service Account)
    ⚠️ ชีทต้องแชร์เป็น "ทุกคนที่มีลิงก์ (ผู้ดู)" ไว้ก่อน ไม่งั้นอ่านไม่ได้
    ...
    try:
        ...
        csv_url = f"https://docs.google.com/spreadsheets/d/{sheet_id}/gviz/tq?tqx=out:csv&sheet={ws_name}"
        resp = requests.get(csv_url, timeout=15)
        if resp.status_code != 200 or "html" in resp.headers.get("Content-Type", ""):
            return [], (
                "อ่านชีทไม่สำเร็จ — เช็คว่าชีทแชร์เป็น 'ทุกคนที่มีลิงก์ (ผู้ดู)' แล้วหรือยัง "
                "และชื่อแท็บถูกต้องไหม"
            )
        ...
        for row in rows[1:]:
            ...
            code = str(row_dict.get("รหัส", "")).strip()
            name = str(row_dict.get("รายชื่อสาขา", "")).strip()
...
def log_to_sheet(reporter, branch, zone, status, reason="", filename="", url="", max_retries=3):
    """
    บันทึกแถวข้อมูลลง Google Sheet ผ่าน Google Apps Script Web App
    (แทนที่ Service Account เดิม — ต้องตั้งค่า st.secrets["apps_script"]["url"]
    และ st.secrets["apps_script"]["secret"] ให้ตรงกับที่ตั้งไว้ใน Code.gs)

    ลำดับคอลัมน์: วันที่เวลา, ผู้กรอก, สาขา, Zone, สถานะ, เหตุผล, ชื่อไฟล์, ลิงก์รูป

    ถ้าเขียนไม่สำเร็จ (เช่น เน็ตสะดุดชั่วคราว) จะลองใหม่อัตโนมัติ
    สูงสุด max_retries ครั้ง โดยเว้นช่วงเพิ่มขึ้นเรื่อยๆ ระหว่างแต่ละครั้ง (1s, 2s, 4s)
    ...
    payload = {
        "secret": st.secrets["apps_script"]["secret"],
    ...
    for attempt in range(1, max_retries + 1):
        try:
            resp = requests.post(st.secrets["apps_script"]["url"], json=payload, timeout=20)
            data = resp.json()
            if data.get("ok"):
                return True, ""
            last_err = data.get("error", "ไม่ทราบสาเหตุ")
        except Exception as e:
            last_err = str(e)
